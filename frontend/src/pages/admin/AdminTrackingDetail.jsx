    ...
    <div className="space-y-6 p-6 max-w-4xl mx-auto">
      ...
      <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-200">
        ...
        <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-300 before:to-transparent">
          ...
          <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
            <div
              className={`flex items-center justify-center w-10 h-10 rounded-full border-4 border-white shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 ${isDelivered ? "bg-blue-500 text-white" : "bg-amber-400 text-white"}`}
            >
              {isDelivered ? "✓" : "⏱"}
            </div>
            <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border border-slate-200 bg-white">
              <div className="flex items-center justify-between space-x-2 mb-1">
                <div className="font-bold text-slate-900">
                  Penerima ({penerima.name || "Penerima"})
                </div>
                <time className="text-xs font-medium text-slate-500">
                  {isDelivered
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
              {shipment.feedback_images.map((img, idx) => (
                <img
                  ...
                  className="w-full h-40 object-cover rounded-xl border bg-slate-50 shadow-sm"
                />
              ))}
            </div>
          ) : (
